...
void loop() {
  if (Serial.available() > 0) {
    char rr = Serial.read(); // читаем порт по строке за одну петлю пока не соберем преамбулу



    if (rr == 0x04 && h_s == 3) {   //собрали преамбулу принимаем 3 байта адреса и длину
      h_s = 0;
      ...
      if (len) {
        ...
        if (crc == 0) {
          ...
          digitalWrite(13, HIGH);  // данные повреждены наливаем еще коньяка и моргаем диодом)))
        }
      }
    }
